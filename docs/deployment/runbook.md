...
| Networks | **Two segregated Docker networks per box** (#506): `${project}_default` (`10.60.0.0/24`, our stack) and `${project}_tks` (`10.61.0.0/24`, the guest TheKittySaver stack). **Caddy is the only container on both** (static IPs `10.60.0.100` / `10.61.0.100`); nothing else crosses. The apps trust `X-Forwarded-*` from Caddy's `10.60.0.100/32` only — keep that /32 in lockstep with Caddy's `ipv4_address`. A third, `${project}_obs`, carries the hand-deployed observability project (ADR-0050 §2): Caddy is on it too, unpinned, only so the ingest vhost can reach the backend containers (#755). |
...
| `FrontendCaller__Key` | — (the limiter is off) | from `FRONTEND_CALLER_KEY` | ✅ non-dev | **secret** | The frontend's caller key (ADR-0054): a frontend call that carries it next to the visitor's address is rate-limited on that visitor, not on the frontend container. ≥ 32 chars; the boot fails without it outside Development/Testing. **The same variable feeds the frontend's `AuthSystem__CallerKey`.** |
...
| `AuthSystem__CallerKey` | — (the auth limiter is off) | from `FRONTEND_CALLER_KEY` | ✅ non-dev | **secret** | Sent with the visitor's address on every auth API call (ADR-0054). Must equal auth-api's `FrontendCaller__Key` — compose reads both from the one variable. ≥ 32 chars; the boot fails without it outside Development/Testing. |
| `ForwardedHeaders__KnownNetworks__0` | — (dev skips `UseForwardedHeaders`) | `10.60.0.100/32` (Caddy's pinned static IP) | optional | plain | See the auth-api row. |
| `OTEL_EXPORTER_OTLP_ENDPOINT` / `OTEL_EXPORTER_OTLP_PROTOCOL` | `http://localhost:4317` / `grpc` (launchSettings) | — (empty: no sink today) | optional | plain | Empty endpoint = export disabled. |
...
vhosts), `DOMAIN_OBS` / `OBS_ALLOWED_PUSH_IP` / `OBS_PUSH_USER` (the observability ingest vhost —
**backend box only**, the other box keeps the inert compose defaults; #755), `XROBOTS` (SEO
crawler control, #531 — Caddy stamps it as `X-Robots-Tag` on every LOTRO vhost response; prod
...
| `FRONTEND_CALLER_KEY` (→ auth-api `FrontendCaller__Key` **and** frontend `AuthSystem__CallerKey`) | auth-api, frontend | **box-local** — generate: `openssl rand -base64 32`, one per environment | Lets the auth API rate-limit a frontend call on the visitor's forwarded address instead of on the frontend container (ADR-0054). Put a new value into the box `.env` and redeploy: both services read the same line, so they disagree only for the seconds of the restart, and a call in that window merely counts against the frontend's own bucket. **Required** — compose refuses to render without it (`deploy.sh` stops at its first gate and the rollback keeps the old release serving), and both apps refuse to boot outside Development/Testing. A leaked key lets its holder dodge the three back-channel limits by inventing addresses and nothing more — rotate it. |
...
7. **The frontend caller key is one value read by two services.** `FRONTEND_CALLER_KEY` in the box
   `.env` feeds auth-api's `FrontendCaller__Key` and the frontend's `AuthSystem__CallerKey`
   (ADR-0054). Every frontend→auth call arrives from the frontend container, so without the key the
   auth API's per-address rate limits are one bucket for every user; with it, the auth API meters the
   visitor the frontend forwards. Compose refuses to render without it, so **the key goes into the
   staging `.env` before a change that needs it merges, and into the prod `.env` before it is
   promoted**. Two services with different values is not a failure, only the old shared bucket — so
   never set it on one side by hand.
...
- **Grafana** — logs, metrics and traces for **both projects** on the stack described below
  (epic #707), from both boxes once the prod agent has been brought up (#755, the procedure is
  below) — until then staging only.
...
2. **Mint the push credential.** One password, two forms, and the two never travel together:
   ...
   docker run --rm -it caddy:2-alpine caddy hash-password   # prompts for it; the hash → staging /opt/lotro/.env
   ```
3. **Staging box, `/opt/lotro/.env`** (as `deploy`; the block is in `.env.hetzner.example`):
   ```
   ...
   It creates `lotro-staging_obs`, attaches Caddy, validates and reloads the Caddyfile, and ACME
...
7. **Prove it in Grafana** (tunnel): Explore → Loki `{box="lotro-prod"}` lists every prod
   container; the LotroKoniecDev dashboard's environment picker offers `production` with data behind
