...
# thing that checks it before it gets there (HETZ-04); bootstrap.sh because its swap leg edits state
# that can stop a box from booting (/etc/fstab), and its self-test is the only thing that sees that
# edit before a box does (OBS-01); the two scripts/ci/ halves of the N-1 promotion gate because CD's
# prod leg runs them (#534) and their self-tests are the only pre-deploy check they face; and
