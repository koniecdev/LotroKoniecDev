...
/// <summary>
/// The patcher's copy of the rule that finds the field boundaries (ADR-0042). Its twin lives in the TMS
/// suite over the TMS' own copy, and <c>ParserContractParityTests</c> checks the two against each other.
/// The contexts share the file and never the code, so nothing else stops one copy from changing on its
/// own.
/// </summary>
