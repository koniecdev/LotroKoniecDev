...
|---|---|---|---|
| **R1** | **HTTP ingress + TLS termination**, one public HTTPS origin per web app (frontend, auth-api, tms-api) | Apps serve **plain HTTP on `:8080`**; the ingress owns TLS. tms-api's origin must be **publicly reachable** for CLI/desktop-app translation-file distribution. | ADR-0008 §2; runbook *container contract* |
