...
/// <summary>
/// Names the client a request is metered as (ADR-0054). The frontend's calls all leave from one
...
/// direct caller, a wrong or missing key, a missing, repeated or broken address, is metered on
/// <c>Connection.RemoteIpAddress</c>, which <c>UseForwardedHeaders</c> resolved from Caddy's pinned /32
/// (#399, #506), so a caller without the key can never choose its bucket. The keys are compared as
/// SHA-256 digests in constant time: a wrong guess learns neither the key's length nor how much of it
/// matched.
/// </summary>
